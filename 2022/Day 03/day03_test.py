...
def preprocessed_input():
    input = load_input_data(file.parent / "example_input.txt", day=3, year=2022)
    return day.preprocess_input(input)


def test_get_priority():
    assert day.get_priority("a") == 1
    assert day.get_priority("z") == 26
    assert day.get_priority("A") == 27
    assert day.get_priority("Z") == 52
